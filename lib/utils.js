...
export function parseAppointmentDate(appt) {
  ...
  if (appt.exactDate) return parseDateString(appt.exactDate);
...
export function getFiltered(appts, state) {
  ...
  if (state.cat && state.cat !== 'all' && state.cat !== "mariees") {
    list = list.filter((a) => (a.service || a.cat) === state.cat);
  }
  if(state.cat && state.cat === "mariees"){
    list = list.filter((a) => ((a.service || a.cat) === state.cat)||((a.service||a.cat)==="deplacement"));
  }
