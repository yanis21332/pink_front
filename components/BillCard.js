...
const PrintButton = styled(Button)`
  ...
  &:hover {
    background: linear-gradient(135deg, rgba(237, 100, 166, 0.15), rgba(237, 100, 166, 0.1));
...
export default function BillCard({ bill, menu }) {
  ...
  const getServiceName = (serviceId) => {
    for (const category of menu) {
      if (category.services && Array.isArray(category.services)) {
        const service = category.services.find((s) => s.id === serviceId || s._id === serviceId);
        if (service) {
      ...
      <CardHeader>
        ...
        <ActionButtons>
          <StatusBadge $status={bill.status}>
            {bill.isPaid === true ? "Payée" : "Non payée"}
          </StatusBadge>
      ...
      <DetailsSection $open={detailsOpen}>
        <DetailGrid>
          <DetailItem>
            ...
            <DetailValue>{bill.totalAmount || 0} DT</DetailValue>
          </DetailItem>
          <DetailItem>
          ...
          <DetailItem>
            ...
            <DetailValue>
              {bill.status === "paid" ? "Payée" : "Non payée"}
            </DetailValue>
          </DetailItem>
        </DetailGrid>
        ...
        {bill.services && bill.services.length > 0 && (
          <ServicesList>
            ...
            <TotalAmount>
              <span>Total</span>
              <span>{bill.totalAmount || 0} DT</span>
            </TotalAmount>
          </ServicesList>
        )}
      </DetailsSection>
